...
var tweets = {};
var count = 0;
var twitter_callback = function(callback) {
    return function(error, data, response){
        ...
        if(data.search_metadata.next_results != undefined){
            T.get('search/tweets',querystring.parse(data.search_metadata.next_results.substring(1)), twitter_callback(callback));
        } else {
            callback();
        }
    }
...
T.get('search/tweets', { q: '#'+process.env.hashtag+' since:'+today+' -RT', count:100 }, twitter_callback(function(){
